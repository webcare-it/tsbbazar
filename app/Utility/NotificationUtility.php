...
class NotificationUtility
{
    ...
    {       
        ...
        try {
            ...
            if ($order->orderDetails->first() && $order->orderDetails->first()->product && $order->orderDetails->first()->product->user) {
        ...
        if ($request != null && get_setting('google_firebase') == 1 && $order->user && !empty($order->user->device_token)) {
    ...
    {        
        ...
        if ($admin && $admin->id && $order->seller_id != $admin->id) {
            $userIds[] = $admin->id;
        }
