...
class CartController extends Controller
{
            ...
            foreach ($owner_ids as $owner_id) {
                ...
                if (!empty($shop_items_raw_data)) {
                    foreach ($shop_items_raw_data as $shop_items_raw_data_item) {
                        $product = Product::where('id', $shop_items_raw_data_item["product_id"])->where('published', 1)->first();
    ...
    {
        $product = Product::where('published', 1)->findOrFail($request->id);
    ...
    public function process(Request $request)
    {
        ...
        if (!empty($cart_ids)) {
            ...
            foreach ($cart_ids as $cart_id) {
                ...
                $product = $cart_item ? Product::where('id', $cart_item->product_id)->where('published', 1)->first() : null;
