    ...
    <div class="card-body">
        <div class="row gutters-5">
            ...
            @php
                ...
                // Used by multiple UI blocks (transfered option + Steadfast button).
            ...
            <div class="col-md-3 ml-auto">
             ...
             @if(!$hasBProductId)
                @if(get_setting('steadfast_api_key') && get_setting('steadfast_secret_key') && $order->delivery_status != 'delivered' && $order->delivery_status != 'cancelled')
            <div class="col-md-3 ml-auto">
                <label>{{translate('Steadfast Courier')}}</label>
                <button type="button" class="btn btn-primary btn-block" onclick="send_to_steadfast()">{{ translate('Send to Steadfast') }}</button>
            </div>
                @endif
            @endif
        ...
        <div class="row gutters-5">
            <div class="col text-center text-md-left">
                <address>
                    <strong class="text-main">{{ optional($shipping_address)->name ?? '' }}</strong><br>
                    {{ optional($shipping_address)->email ?? '' }}<br>
                    {{ optional($shipping_address)->phone ?? '' }}<br>
                    {{ optional($shipping_address)->address ?? '' }}
                    @if(optional($shipping_address)->city || optional($shipping_address)->state || optional($shipping_address)->country)
                        , {{ optional($shipping_address)->city ?? '' }} {{ optional($shipping_address)->state ?? '' }} {{ optional($shipping_address)->country ?? '' }}
                    @endif
                    <br>
                    @if(optional($shipping_address)->postal_code)
                        {{ optional($shipping_address)->postal_code }}
                    @endif
                </address>
                        ...
                        <tr>
                            ...
                            <td>
                                @if ($orderDetail->product != null && $orderDetail->product->auction_product == 0)
                                    ...
                                    @endphp
                                    <img height="50" src="{{ uploaded_asset($productImage) }}">
                            ...
                            <td>
                                @if ($orderDetail->product != null && $orderDetail->product->auction_product == 0)
                                    <strong><a href="{{ env('APP_URL') . 'products/' . $orderDetail->product->id .'/' . $orderDetail->product->slug }}" target="_blank" class="text-muted">{{ $orderDetail->product->getTranslation('name') }}</a></strong>
...
@section('modal')
    @include('modals.steadfast_modal', ['order_id' => $order->id])
@endsection
...
@section('script')
    <script type="text/javascript">
        ...
        $(document).on('submit', '#steadfast-form', function(e){
            e.preventDefault();

            var $form = $(this);
            var $confirmBtn = $('#steadfast-confirm-btn');
            $confirmBtn.prop('disabled', true);

            $.post($form.attr('action'), $form.serialize(), function(data){
                if(data.success) {
                    AIZ.plugins.notify('success', data.message);
                    $('#steadfast-modal').modal('hide');
                    setTimeout(function() {
                        location.reload();
                    }, 1000);
                } else {
                ...
                AIZ.plugins.notify('danger', '{{ translate('Error sending to Steadfast') }}');
            }).always(function() {
        ...
        function send_to_steadfast() {
            $('#steadfast-order-id').val({{ $order->id }});
            $('#steadfast-modal').modal('show');
        }
    </script>
@endsection
