            ...
            <div class="row">
                <div class="col-lg-8">
                    <form action="{{ route('payment.checkout') }}" class="form-default" role="form" method="POST"
                        ...
                        <!-- Hidden phone field for OTP verification -->
                        <input type="hidden" name="phone" value="{{ $shipping_info ? $shipping_info->phone : '' }}">
...
@section('script')
    <script type="text/javascript">
        ...
        // OTP System for Checkout
        var isOtpVerified = false;
        var otpVerificationNeeded = {{ get_setting('otp_for_order') == 1 ? 'true' : 'false' }};

        function submitOrder(el) {
            // If OTP is not needed, proceed with normal submission
            if (!otpVerificationNeeded) {
                submitOrderOriginal(el);
                return;
            }

            // If OTP is already verified, proceed with normal submission
            if (isOtpVerified) {
                submitOrderOriginal(el);
                return;
            }
            ...
            if (!phone) {
                AIZ.plugins.notify('danger', '{{ translate('Please provide a phone number for OTP verification') }}');
                $(el).prop('disabled', false);
                return;
            }

            // Send OTP
            sendOtp(phone, function(success) {
                if (success) {
                    // Show OTP modal
                    showOtpModal(phone, el);
                } else {
                    $(el).prop('disabled', false);
                }
            });
        ...
        function submitOrderOriginal(el) {
            $(el).prop('disabled', true);
            if ($('#agree_checkbox').is(":checked")) {
                if (minimum_order_amount_check && $('#sub_total').val() < minimum_order_amount) {
                    AIZ.plugins.notify('danger',
        ...
        function sendOtp(phone, callback) {
            $.ajax({
                ...
                error: function() {
                    AIZ.plugins.notify('danger', '{{ translate('Failed to send OTP') }}');
                    callback(false);
                }
            });
        }

        function verifyOtp(phone, otpCode, el) {
            $.ajax({
                headers: {
                    'X-CSRF-TOKEN': $('meta[name="csrf-token"]').attr('content')
                },
                method: "POST",
                url: "{{ url('/api/v2/order/verify-otp') }}",
                data: {
                    phone: phone,
                    otp_code: otpCode
                },
                success: function(data) {
                    if (data.result) {
                        ...
                        $('#checkout-form').append('<input type="hidden" name="otp_code" value="' + otpCode + '">');
                ...
                error: function() {
                    AIZ.plugins.notify('danger', '{{ translate('Failed to verify OTP') }}');
                }
            });
        }

        function showOtpModal(phone, el) {
            ...
            if ($('#otp-modal').length === 0) {
                        ...
                        <div class="modal-dialog modal-dialog-centered" role="document">
                            <div class="modal-content">
                                <div class="modal-header">
                                    <h5 class="modal-title">{{ translate('OTP Verification') }}</h5>
                                    <button type="button" class="close" data-dismiss="modal" aria-label="Close">
                                        <span aria-hidden="true">&times;</span>
                                    </button>
                                </div>
                                <div class="modal-body">
                                    <p>{{ translate('Please enter the OTP code sent to your phone number') }}: <strong>` + phone + `</strong></p>
                                    <div class="form-group">
                                        <label for="otp-code">{{ translate('OTP Code') }}</label>
                                        <input type="text" id="otp-code" class="form-control" placeholder="{{ translate('Enter OTP code') }}">
                                    </div>
                                </div>
                                <div class="modal-footer">
                                    <button type="button" class="btn btn-secondary" data-dismiss="modal">{{ translate('Cancel') }}</button>
                                    <button type="button" class="btn btn-primary" id="verify-otp-btn">{{ translate('Verify OTP') }}</button>
                                </div>
                            </div>
                        </div>
                    </div>
                ...
                $(document).on('click', '#verify-otp-btn', function() {
                    ...
                    if (otpCode) {
                        verifyOtp(phone, otpCode, el);
                    } else {
                        AIZ.plugins.notify('danger', '{{ translate('Please enter OTP code') }}');
                    }
                });
            }
